class Carousel {
    ...
    createNavigation () {
      ...
      this.onMove(index => {
        if (index === 0) {
          prevButton.classList.add('pdb-carrousel-prev-hidden')
        } else {
          prevButton.classList.remove('pdb-carrousel-prev-hidden')
        }
        if (this.items[this.currentItem + this.slidesVisible] === undefined) {
          nextButton.classList.add('pdb-carrousel-next-hidden')
        } else {
          nextButton.classList.remove('pdb-carrousel-next-hidden')
        }
      })
    }
     ...
     */
    createPagination () {
      let pagination = this.createDivWithClass('pdb-carrousel-pagination')
      let buttons = []
      this.root.appendChild(pagination)
      for (let i = 0; i < (this.items.length - 2 * this.offset); i = i + this.options.slidesToScroll) {
        let button = this.createDivWithClass('pdb-carrousel-pagination-button')
      ...
      this.onMove(index => {
        ...
        if (activeButton) {
          buttons.forEach(button => button.classList.remove('pdb-carrousel-pagination-button-active'))
          activeButton.classList.add('pdb-carrousel-pagination-button-active')
        }
      })
    }
